...
book: common
	...
	@for i in `seq $(N_INCLUDE_MIN) $(N_INCLUDE_MAX)`; do echo -e $(SUB_BOOK) > $(NAME)/sub/part1/chapter1/section$$i.tex; done

report: common
	@./generate.sh report > $(NAME)/main.tex
	@mkdir $(NAME)/sub/chapter1
	@for i in `seq $(N_INCLUDE_MIN) $(N_INCLUDE_MAX)`; do echo -e $(SUB_REPORT) > $(NAME)/sub/chapter1/section$$i.tex; done

article: common
	@./generate.sh article > $(NAME)/main.tex
	@mkdir $(NAME)/sub/part1
	@for i in `seq $(N_INCLUDE_MIN) $(N_INCLUDE_MAX)`; do echo -e $(SUB_ARTICLE) > $(NAME)/sub/part1/section$$i.tex; done
...
beamer: common
	...
	@for i in `seq $(N_INCLUDE_MIN) $(N_INCLUDE_MAX)`; do echo -e $(SUB_BEAMER) > $(NAME)/sub/section$$i.tex; done
